// this for add file flexibly

// import React, { useState, useEffect } from 'react';
// import axios from 'axios';
// import examData from './examDauThau.json';
// // --- Utility Functions ---
...
//   useEffect(() => {
//     // Không cần localStorage, không cần axios, dùng luôn data import ở trên
//     const chunks = chunkArray(examData, 30);
//     setMockTests(chunks);
//   }, []);
...
// this for exam dauthau
import React, { useState, useEffect } from 'react';
import examData from './examDauThau.json'; // Nhúng trực tiếp file dữ liệu
...
function App() {
  // Trạng thái khởi đầu giờ là MENU luôn, bỏ qua UPLOAD
  ...
  // --- Tự động load và chia nhỏ đề thi khi vừa mở web ---
  useEffect(() => {
    // Không cần gọi API nữa, lấy luôn examData chia thành các phần 30 câu
  ...
  if (currentStep === 'MENU') {
    return (
      <div className="min-h-screen bg-[#f0ebf8] flex flex-col items-center p-6 py-12">
        <div className="max-w-3xl w-full">
          <div className="bg-white rounded-lg shadow-md p-8 border-t-8 border-t-[#673ab7] mb-6">
            <h1 className="text-3xl font-normal text-gray-800 mb-2">Danh sách Đề thi (Mock Tests)</h1>
            <p className="text-gray-600">Tổng số câu hỏi: {examData.length}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {mockTests.map((test, index) => (
              <div key={index} className="bg-white rounded-lg shadow-sm p-6 border border-gray-200 hover:shadow-md transition-shadow">
                <h3 className="text-xl text-gray-800 mb-2">Mock Test {index + 1}</h3>
                <p className="text-sm text-gray-500 mb-4">{test.length} câu hỏi (Trộn ngẫu nhiên)</p>
                <button 
                  onClick={() => startTest(index)}
                  className="text-[#1a73e8] font-medium hover:bg-blue-50 px-4 py-2 rounded transition-colors -ml-4"
                >
                  Bắt đầu làm bài
                </button>
              </div>
    ...
    return (
      <div className="min-h-screen bg-[#f0ebf8] flex flex-col items-center py-12 p-4">
        <div className="max-w-3xl w-full">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 border-t-8 border-t-[#673ab7] mb-4">
            <h1 className="text-2xl font-normal text-gray-800">Đang thi: Mock Test</h1>
            <p className="text-sm text-red-600 mt-2">* Bắt buộc</p>
          </div>

          <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200 mb-4">
            <div className="flex justify-between items-start mb-6">
              <h2 className="text-base text-gray-800 font-medium" dangerouslySetInnerHTML={{ __html: `${currentIndex + 1}. ${q.questionText}` }}></h2>
              <span className="text-red-500 text-xl ml-2">*</span>
            </div>
            ...
            {isRevealed && (
              <div className="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center">
                <span className={`font-medium ${selectedOption.isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                  {selectedOption.isCorrect ? 'Tuyệt vời! Chính xác.' : 'Rất tiếc, chưa chính xác.'}
                </span>
                <button 
                  onClick={handleNextQuestion}
                  className="bg-[#1a73e8] hover:bg-blue-600 text-white font-medium py-2 px-6 rounded transition-colors shadow-sm"
                >
                  {currentIndex + 1 === currentTestQuestions.length ? 'Nộp bài' : 'Câu tiếp theo'}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-between items-center px-2">
            <span className="text-sm text-gray-500 font-medium">Câu {currentIndex + 1} / {currentTestQuestions.length}</span>
            <button onClick={() => setCurrentStep('MENU')} className="text-sm text-gray-500 hover:underline">Thoát bài thi</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-[#f0ebf8] flex flex-col items-center py-12 p-6">
        <div className="max-w-3xl w-full">
          ...
          <div className="bg-white rounded-lg shadow-sm p-8 border-t-8 border-t-[#673ab7] mb-6">
            <h1 className="text-3xl font-normal text-gray-800 mb-2">Đã hoàn thành bài thi</h1>
            <p className="text-gray-600 mb-6 border-b border-gray-200 pb-4">Câu trả lời của bạn đã được ghi lại.</p>
            
            <div className="flex items-center justify-between">
              <div className="text-xl">
                Điểm số: <span className="font-bold text-3xl text-[#1a73e8]">{score}</span> / {currentTestQuestions.length}
              </div>
              <button 
                onClick={() => setCurrentStep('MENU')}
                className="text-[#1a73e8] font-medium hover:underline"
          ...
          {incorrectAnswers.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl text-gray-800 mb-4 px-2 font-medium">Xem lại các câu trả lời sai:</h2>
              {incorrectAnswers.map((item, index) => (
                <div key={index} className="bg-white rounded-lg shadow-sm p-6 border-l-4 border-l-red-500">
                   <h3 className="text-base text-gray-800 mb-4 font-medium" dangerouslySetInnerHTML={{ __html: item.questionText }}></h3>
                   
                   <div className="bg-red-50 text-red-800 p-3 rounded mb-2 text-sm border border-red-100 flex items-start">
                     <span className="font-bold mr-2 w-24 flex-shrink-0">Bạn chọn:</span> 
                     <span dangerouslySetInnerHTML={{ __html: item.userAnswer }}></span>
                   </div>
                   <div className="bg-green-50 text-green-800 p-3 rounded text-sm border border-green-100 flex items-start">
                     <span className="font-bold mr-2 w-24 flex-shrink-0">Đáp án đúng:</span> 
                     <span dangerouslySetInnerHTML={{ __html: item.correctAnswer }}></span>
                   </div>
                </div>
              ))}
            </div>
          ...
          {incorrectAnswers.length === 0 && (
            <div className="bg-green-50 border border-green-200 text-green-800 rounded-lg p-6 text-center font-medium shadow-sm">
               Tuyệt cú mèo! Bạn đã đúng 100% tất cả các câu. 🎉
            </div>
          )}
        </div>
      </div>
